...
def plot_all_data(dates, df_indicators, actual_values, train_predictions, test_predictions, 
    ...
    fig.add_trace(
        go.Scatter(x=future_dates, y=future_predictions.flatten(),
                  name='Future Predictions', line=dict(color='green')), row=1, col=1)
...
def main():
    ...
    with left_col:
        ...
        with col2:
            end_date = st.date_input("End Date", pd.to_datetime("2024-10-27"))
    ...
    with right_col:
        ...
        lstm_units_1 = st.slider("First LSTM Layer Units:", 20, 150, 64, 2)
        lstm_units_2 = st.slider("Second LSTM Layer Units:", 20, 150, 32, 2)
        ...
        try:
            with st.spinner("Loading data and making predictions..."):
                ...
                with col1:
                    ...
                    # Відображення метрик
                    st.subheader("LSTM Test Metrics 🧪")
